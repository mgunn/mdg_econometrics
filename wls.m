function [results, options] = wls(varargin),
% Author: Matthew Gunn
% Notes:  In general, behavior is designed to match that of Stata
%
%                               ** USAGE ** 
% [results, options_used] = regress(y, X, weights, <option>, <option_val>, ...)
%
% options:                   values:
...
warning('wls is NOT tested yet! experimental');
...
R = chol(XprimWX);   % Cholesky decomposition:    XprimX_chol_UT' * XprimX_chol_UT = XprimX
                    % 
                    % We want to solve: XprimX * b = Xprimy
                    %                     R'*R * b = Xprimy
                    %                            b = R\(R'\Xprimy)

results.b = R\(R'\XprimWy);
